...
当前 `backend/mcp_tools/builtin.py` 共注册 12 个工具：`system`、`process`、
`process.kill`、`network`、`log`、`service`、`service.restart`、`temp.clean`、
`disk`（系统感知与受控操作，详见 `docs/system-perception-tools.md` 和
`docs/controlled-operation-tools.md`），以及 `auth`、`firewall`、`privilege`
三个只读安全态势感知工具（`category="security"`，详见
`docs/security-posture-tools.md`）。新增或调整工具时应同步更新这三份文档。
...
- `GET /api/tools`：查看工具列表和 manifest，当前共 12 个工具。
